...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE reservations (id INTEGER PRIMARY KEY AUTOINCREMENT, client_id INTEGER, voiture_id INTEGER, date_debut TEXT, date_fin TEXT, prix_total REAL, statut TEXT DEFAULT 'en_attente', created_at TEXT)");
        ...
        db.execSQL("INSERT INTO utilisateurs (username, password, role) VALUES ('admin', 'admin', 'admin')");
    }
    ...
    public Utilisateur getUtilisateur(String username, String password) {
        ...
        android.database.Cursor cursor = db.rawQuery("SELECT * FROM utilisateurs WHERE username = ? AND password = ?", new String[]{username, password});
        if (cursor != null) {
            try {
    ...
    public List<Voiture> getVoituresLouees() {
        List<Voiture> voitures = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        android.database.Cursor cursor = db.rawQuery("SELECT * FROM voitures WHERE disponible = 0", null);
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    do {
    ...
    public String getDateFinLocation(int voitureId) {
        ...
        android.database.Cursor cursor = db.rawQuery(
            "SELECT date_fin FROM reservations WHERE voiture_id = ? AND date_fin >= date('now') ORDER BY date_fin ASC LIMIT 1",
            new String[]{String.valueOf(voitureId)});
        String dateFin = null;
        if (cursor.moveToFirst()) {
